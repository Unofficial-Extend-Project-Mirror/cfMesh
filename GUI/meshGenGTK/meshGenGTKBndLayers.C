...
static void updateBndLayers(GtkWidget* bndLayersFramePtr)
{
	...
	GtkWidget* comboboxentry_bndLayers_selectedPatches =
		(GtkWidget*)g_object_get_data
		(
			G_OBJECT(bndLayersFramePtr),
...
static void addBndLayersPatches(GtkWidget* widget, GtkWidget* bndLayersFramePtr)
{
	GtkWidget* comboboxentry_bndLayers_availablePatches =
		(GtkWidget*)g_object_get_data
		(
	...
	guiPtr->addBndLayersPatches(name);
	
	updateBndLayers(bndLayersFramePtr);
}

static void removeBndLayersPatches
(
	GtkWidget* widget,
	GtkWidget* bndLayersFramePtr
)
{
	GtkWidget* comboboxentry_bndLayers_selectedPatches =
		(GtkWidget*)g_object_get_data
	...
	guiPtr->removeBndLayersPatches(name);
...
void meshGenGTK::createBndLayersWindowPage()
{
	guiPtr = &meshGui_;
	
	bndLayersFramePtr_ = gtk_frame_new(NULL);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	gtk_container_add
	(
		GTK_CONTAINER(bndLayersFramePtr_),
		table1
	);

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	//- set label for available patches
	GtkWidget* label_bndLayers_availablePatches =
		gtk_label_new("Available patches");
	gtk_widget_show(label_bndLayers_availablePatches);
	gtk_table_attach
	(
		GTK_TABLE(table1),
		label_bndLayers_availablePatches,
		0, 1, 0, 1,
	...
	(
		GTK_MISC(label_bndLayers_availablePatches),
		0,
		0.5
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	(
		GTK_EDITABLE
		(
			GTK_COMBO(comboboxentry_bndLayers_availablePatches)->entry
		),
		0
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	(
		G_OBJECT(button1),
		"clicked",
		G_CALLBACK(addBndLayersPatches),
		bndLayersFramePtr_
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	GtkWidget* label_bndLayers_selectedPatches =
		gtk_label_new("Selected patches");
	gtk_widget_show(label_bndLayers_selectedPatches);
	gtk_table_attach
	(
		GTK_TABLE(table1),
		label_bndLayers_selectedPatches,
		0, 1, 1, 2,
		(GtkAttachOptions)(GTK_FILL), (GtkAttachOptions)(0), 0, 0
	);
	gtk_misc_set_alignment
	(
		GTK_MISC(label_bndLayers_selectedPatches),
		0,
		0.5
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	GtkWidget* comboboxentry_bndLayers_selectedPatches =
		gtk_combo_new();
	gtk_widget_show(comboboxentry_bndLayers_selectedPatches);
	gtk_table_attach
	(
		GTK_TABLE(table1),
		comboboxentry_bndLayers_selectedPatches,
	...
	(
		GTK_EDITABLE
		(
			GTK_COMBO(comboboxentry_bndLayers_selectedPatches)->entry
		),
		0
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	(
		G_OBJECT(button2),
		"clicked",
		G_CALLBACK(removeBndLayersPatches),
		bndLayersFramePtr_
	);
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
	...
	updateBndLayers(bndLayersFramePtr_);
	
	gtk_widget_show(bndLayersFramePtr_);
}
